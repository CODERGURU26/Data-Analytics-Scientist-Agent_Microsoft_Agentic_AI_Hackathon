...
const API_CONFIGURED = Boolean(import.meta.env.VITE_API_BASE_URL || import.meta.env.VITE_API_ORIGIN)
...
async function requestWithRetry(input: RequestInfo | URL, init?: RequestInit, attempts = 3): Promise<Response> {
...
export async function checkAnalysisConnection(): Promise<boolean> {
  try {
    const response = await requestWithRetry(`${API_ORIGIN}/health`, undefined, 2)
    return response.ok
  } catch {
    return false
  }
...
export async function fetchAnalysis(analysisId: string): Promise<AnalysisStatus> {
